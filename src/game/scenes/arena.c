...
void arena_render_overlay(scene *scene) {
    ...
    if(obj[0] != NULL && obj[1] != NULL) {
        ...
        if(player[0]->chr) {
            ...
            player1_name = lang_get2_offset(LANG2_STR_PILOT, player[0]->pilot->pilot_id);
            player2_name = lang_get2_offset(LANG2_STR_PILOT, player[1]->pilot->pilot_id);
        }

        text_render(&tconf_players, TEXT_DEFAULT, 5, 19, 250, 6, player1_name);
        text_render(&tconf_players, TEXT_DEFAULT, 5, 26, 250, 6,
                    lang_get2_offset(LANG2_STR_ROBOT, player[0]->pilot->har_id));

        if(player[1]->pilot) {
            // when quitting, this can go null
            int p2len = (strlen(player2_name) - 1) * font_small.w;
            // XXX TODO: Magnus: Hardcoded - 1 for trailing newlines in localization strings
            int h2len = (strlen(lang_get2_offset(LANG2_STR_ROBOT, player[1]->pilot->har_id)) - 1) * font_small.w;
            ...
            text_render(&tconf_players, TEXT_DEFAULT, 315 - h2len, 26, 100, 6,
                        lang_get2_offset(LANG2_STR_ROBOT, player[1]->pilot->har_id));
        }
...
int arena_create(scene *scene) {
    ...
    if(scene->gs->init_flags->record == 1) {
        ...
        for(int i = 0; i < 2; i++) {
            ...
            // name in here?
            strncpy(local->rec->pilots[i].info.name, lang_get2_offset(LANG2_STR_PILOT, player->pilot->pilot_id), 18);
        }
