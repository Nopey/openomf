...
void lab_dash_main_update(scene *s, dashboard_widgets *dw) {
    ...
    label_set_text(dw->har_name, lang_get2_offset(LANG2_STR_ROBOT, p1->pilot->har_id));
    label_set_text(dw->har_moves, lang_get2_offset(LANG2_STR_ROBOT_DESCRIPTION, p1->pilot->har_id));
...
void lab_dash_sim_update(scene *s, dashboard_widgets *dw, sd_pilot *pilot) {
    ...
    label_set_text(dw->losses, tmp);
    snprintf(tmp, sizeof(tmp), "MODEL: %s", lang_get2_offset(LANG2_STR_ROBOT, pilot->har_id));
